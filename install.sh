...
if [ -f /etc/os-release ]; then
    ...
    echo "Error: OS tidak terdeteksi!"
    exit 1
fi

# 3. Validasi OS
if [[ "$OS" == "debian" && ("$VERSION" == "10" || "$VERSION" == "11" || "$VERSION" == "12") ]] || \
   [[ "$OS" == "ubuntu" && ("$VERSION" == "20.04" || "$VERSION" == "22.04") ]]; then
    echo "OS Terdeteksi: $OS $VERSION. Melanjutkan..."
else
    echo "Error: Skrip ini hanya mendukung Debian 10/11/12 dan Ubuntu 20.04/22.04."
    exit 1
fi

# 4. Update & Install Paket Dasar
...
echo -e "${CYAN}=================================================${NC}"
echo -e "${YELLOW}           Script by Houzeen x Chuun                 ${NC}"
echo -e "${CYAN}=================================================${NC}"
echo -e "${YELLOW}           PILIHAN JENIS DOMAIN                  ${NC}"
echo -e "${CYAN}=================================================${NC}"
echo -e "${CYAN}│${NC} [1] Gunakan Domain Random HOUZEEN x CHUUN (Gratis/Auto)"
...
(
    ...
    sed -i '/Welcome To HOUZEEN x CHUUN/d' /root/.bashrc
    sed -i '/clear/d' /root/.bashrc
    echo "clear" >> /root/.bashrc
    echo "neofetch" >> /root/.bashrc
    echo 'echo -e "Welcome To Houzeen x Chuun Auto Script! Type \e[1;32mmenu\e[0m to start."' >> /root/.bashrc
) >/dev/null 2>&1 & install_spin
...
<font color="#FFD700">Owner   :</font> <font color="#FFFFFF">Houzeen x Chuun</font><br>
<font color="#FFD700">Telegram:</font> <font color="#FFFFFF">@Houzeen</font><br>
<font color="#00FFFF">──────────────────────────────</font><br>
...
function header_main() {
    ...
    echo -e "                Owner     :  ${WHITE}Houzeen x Chuun${NC}       "
    echo -e "                Telegram  :  ${WHITE}@Houzeen${NC}         "
...
def send_welcome(message):
    ...
    btn_admin = types.InlineKeyboardButton("📞 Hubungi Admin", url="https://t.me/Houzeen")
...
def callback_query(call):
    ...
    if call.data == "menu_xray":
        ...
        markup.add(
            ...
            types.InlineKeyboardButton("📞 Hubungi Admin", url="https://t.me/Houzeen")
        )
...
•────────────────────•"""
        bot.send_photo(chat_id, "https://i.postimg.cc/9QXppGXs/Kode-QRIS-HOUZEEN x CHUUN-Store-Jepara.png", caption=donasi_text)
...
[Unit]
Description=HOUZEEN x CHUUN Telegram Client Bot
After=network.target
...
echo -e "\n${GREEN}=================================================${NC}"
echo -e "${YELLOW} Instalasi Selesai! Membuka Menu... ${NC}"
echo -e "${GREEN}=================================================${NC}\n"
    
echo "Instalasi selesai!"
echo "Menjalankan menu..."
# Baris ini akan otomatis mengganti nama setiap kali skrip dijalankan
sed -i 's/TENDO STORE/HOUZEEN X CHUUN/g' /usr/bin/menu
# Otomatis ganti Branding
menu
